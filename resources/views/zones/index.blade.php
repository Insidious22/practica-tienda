...
@section('content')
<div class="header">
    <h1 class="title">Gestion de Zonas</h1>
...
@if ($zones->count() === 0)
    <div class="empty-state">
        <div class="empty-state-icon">Zonas</div>
        ...
        @foreach ($zones as $zone)
            <div class="zone-card">
                <div class="zone-card-header">
                    ...
                    <span class="muted zone-card-code">Codigo: <strong>{{ $zone->code }}</strong></span>
                </div>
                <div class="zone-card-body">
                    ...
                    <div class="zone-card-footer">
                        <div class="muted zone-card-meta">
                            {{ $zone->categories->count() }} categorias
                        </div>
                        <div class="zone-card-actions">
                            <a class="btn btn-secondary zone-card-action" href="{{ route('admin.zonas.show', $zone) }}">Ver</a>
                            <a class="btn btn-secondary zone-card-action" href="{{ route('admin.zonas.edit', $zone) }}">Editar</a>
                            <form action="{{ route('admin.zonas.destroy', $zone) }}" method="POST" class="zone-card-action">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger zone-card-action-button" type="submit" onclick="return confirm('Eliminar esta zona?')">Eliminar</button>
                            </form>
                        </div>
                    </div>
                </div>
